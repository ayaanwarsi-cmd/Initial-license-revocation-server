# app.py
"""
License Management Server — Admin UI restricted to Revoke-only feature.
- Programmatic APIs (sign, issue, revoke, status, signed download, CSV export) are unchanged.
- Admin web UI only allows:
    - login
    - revoke a license (by ID + reason)
    - view issued licenses table + Download signed license
...
# ---------------- Admin UI templates (revoke-only UI) ----------------
_LOGIN_HTML = """
...
_ADMIN_HTML_REVOKE_ONLY = """
<!doctype html><html><head><meta charset="utf-8"><title>Admin — Revoke</title>
<style>
...
.table th,.table td{padding:8px;border-bottom:1px solid #0b394f;text-align:left;vertical-align:top}
.small{color:#9fb2c8}
.search{display:flex;gap:8px;margin-bottom:8px}
.input{padding:8px;border-radius:6px;border:1px solid #123;background:#03121a;color:#dff}
</style></head><body>
<div class="header">
  <div><strong>Admin — Revoke License</strong></div>
  <div>
...
<div class="card">
  ...
  <form method="post" action="{{ url_for('admin_revoke') }}">
    <div style="display:flex;gap:8px;align-items:center">
      <div style="flex:1">
        <label class="small">License ID</label>
...
<div class="card" style="margin-top:12px">
  ...
  <div class="small">Use search to filter. Clicking Download will always return a signed license JSON (generated if not saved).</div>
  <form method="get" action="{{ url_for('admin') }}" style="margin-top:8px">
    <div class="search">
      <input class="input" name="q" placeholder="search license id / email / hwid" value="{{ q or '' }}">
  ...
  {% if licenses %}
    <table class="table">
      <thead><tr><th>ID</th><th>Email</th><th>HWID</th><th>Expiry</th><th>Status</th><th>Actions</th></tr></thead>
      <tbody>
        {% for L in licenses %}
        <tr>
...
<div class="card" style="margin-top:12px">
  ...
  {% if revoked %}
    ...
    <tbody>
      {% for R in revoked %}
        <tr><td><code>{{ R.license_id }}</code></td><td>{{ R.revoked_at or '-' }}</td><td>{{ R.revoked_reason or '-' }}</td><td>{{ R.revoked_by or '-' }}</td></tr>
      {% endfor %}
    </tbody></table>
  {% else %}
...
# ---------------- CSV export & rotate (APIs remain available) ----------------
...
# ---------------- Compatibility & sign API (unchanged) ----------------
